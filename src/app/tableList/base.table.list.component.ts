...
export class BaseTableListComponent implements OnChanges{
  ...
  ngOnChanges(changes: {[propKey: string]: SimpleChange}){
    ...
    for (let propName in changes) {
      ...
      if(this.tableListConfig.query&&this.tableListConfig.query.queryElements){
        this.tableListConfig.query.queryElements.forEach((queryItem:any)=>{
  ...
  queryDataByPage(param:any) {
    ...
    if(this.tableListConfig&&this.tableListConfig.query&&this.tableListConfig.query.beforeQuery&&this.tableListConfig.query.beforeQuery.length){
      this.tableListConfig.query.beforeQuery.forEach((checkQueryFunc:any)=>{
        if(checkResult){
          if(typeof checkQueryFunc == "function"){
            checkResult=checkQueryFunc(param);
          }
        }
      })
    }
  ...
  listData(){
    let url:string=this.tableListConfig.url;
    this.baseDataService.listData({url,param:this.queryParam,httpMethod:this.tableListConfig.httpMethod,baseUrl:this.tableListConfig.baseUrl}).subscribe((data:any)=>{
